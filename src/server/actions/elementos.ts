...
export async function elementoEstaReferenciado(id: string): Promise<boolean> {
  const [comoIngrediente, comoResultado, ingredienteDeAvance, secuencia] = await Promise.all([
  ...
  return comoIngrediente + comoResultado + ingredienteDeAvance + secuencia > 0
}
...
export async function guardarElemento(
  ...
  try {
    ...
    if (id) {
      const actual = await prisma.element.findUnique({ where: { id } })
      if (!actual) return { ok: false, error: 'El elemento no existe.' }
      if (actual.slug !== d.slug && (await elementoEstaReferenciado(id))) {
        return {
          ok: false,
          error:
            'El identificador (slug) es inmutable porque este elemento ya es usado por recetas, secuencias o jugadores.',
        }
      }
      await prisma.element.update({ where: { id }, data: { ...data, slug: d.slug } })
      elementId = id
    } else {
      const creado = await prisma.element.create({ data: { ...data, slug: d.slug } })
      elementId = creado.id
    }
...
export async function eliminarElemento(id: string): Promise<EstadoAccion> {
  try {
    ...
    // se retira también la definición completa del avance.
    await prisma.$transaction(async (tx) => {
